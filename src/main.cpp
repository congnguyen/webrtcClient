...
// }
// // Global queues for video and audio packets
...
// std::condition_variable videoQueueCondVar;
// std::condition_variable audioQueueCondVar;

// // Function to save packets to files
// void savePacketToFile(AVPacket* packet, std::ofstream& file) {
//     file.write(reinterpret_cast<const char*>(packet->data), packet->size);
// }
// void transcodeStream(const std::string &rtspUrl) {
...
//     if (avformat_open_input(&fmt_ctx, rtspUrl.c_str(), nullptr, nullptr) < 0) {
...
//     // Open output files
//     std::ofstream videoFile("output.264", std::ios::binary);
//     std::ofstream audioFile("ouput.opus", std::ios::binary);

//     if (!videoFile.is_open() || !audioFile.is_open()) {
...
//             video_stream_idx = i;
//             const AVCodec *video_codec = avcodec_find_decoder(fmt_ctx->streams[i]->codecpar->codec_id);
//             video_codec_ctx = avcodec_alloc_context3(video_codec);
//             avcodec_parameters_to_context(video_codec_ctx, fmt_ctx->streams[i]->codecpar);
...
//             video_enc_ctx->width = video_codec_ctx->width;
//             video_enc_ctx->height = video_codec_ctx->height;
...
//             avcodec_open2(video_enc_ctx, video_encoder, nullptr);
//         }
//         if (fmt_ctx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && audio_stream_idx == -1) {
//             audio_stream_idx = i;
//             const AVCodec *audio_codec = avcodec_find_decoder(fmt_ctx->streams[i]->codecpar->codec_id);
//             audio_codec_ctx = avcodec_alloc_context3(audio_codec);
//             avcodec_parameters_to_context(audio_codec_ctx, fmt_ctx->streams[i]->codecpar);
//             avcodec_open2(audio_codec_ctx, audio_codec, nullptr);

//             const AVCodec *audio_encoder = avcodec_find_encoder(AV_CODEC_ID_OPUS);
...
//            audio_enc_ctx->ch_layout = audio_codec_ctx->ch_layout; // Use the new channel layout
//            audio_enc_ctx->strict_std_compliance = FF_COMPLIANCE_EXPERIMENTAL;
//            AVDictionary* options = nullptr;
//             av_dict_set(&options, "frame_size", "960", 0);
//             avcodec_open2(audio_enc_ctx, audio_encoder, &options);
//         }
//     }
...
//                 while (avcodec_receive_frame(video_codec_ctx, origin_frame) == 0) {
           
//                     avcodec_send_frame(video_enc_ctx, origin_frame);
//                     while (avcodec_receive_packet(video_enc_ctx, &enc_packet) == 0) {
//                         std::unique_lock<std::mutex> locker(videoQueueMutex);
//                         // videoQueue.push(enc_packet);
//                         savePacketToFile(&enc_packet, videoFile);
...
//                     while (avcodec_receive_packet(audio_enc_ctx, &enc_packet) == 0) {
//                         std::unique_lock<std::mutex> locker(audioQueueMutex);
//                         // audioQueue.push(enc_packet);
//                         printf("audio packet size: %d\n", enc_packet.size);
//                         savePacketToFile(&enc_packet, audioFile);
...
//     std::string rtspUrl = "rtsp://10.8.0.82/sample.webm";//argv[1];
//     std::thread transcoderThread(transcodeStream, rtspUrl);
...
//             audio_enc_ctx->bit_rate = 64000;
//             audio_enc_ctx->sample_fmt = AV_SAMPLE_FMT_FLT;
//             audio_enc_ctx->sample_rate = audio_codec_ctx->sample_rate;
//             audio_enc_ctx->channel_layout = audio_codec_ctx->channel_layout;
//             audio_enc_ctx->channels = av_get_channel_layout_nb_channels(audio_enc_ctx->channel_layout);
//             audio_enc_ctx->frame_size = 960; // Set the frame size to match typical Opus frame sizes
//             audio_enc_ctx->strict_std_compliance = FF_COMPLIANCE_EXPERIMENTAL;
//             audio_enc_ctx->sample_rate = 48000;

//             avcodec_open2(audio_enc_ctx, audio_encoder, nullptr);
//         }
//     }
...
// int main(int argc, char *argv[]) {
//     std::string rtspUrl = "rtsp://10.8.0.82/sample.webm";
//     std::thread transcoderThread(transcodeStream, rtspUrl);
//     transcoderThread.join();
